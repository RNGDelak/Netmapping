#!/usr/bin/env python3
"""
net_mapping_lite.py — slimmed-down crawler UI (lightweight)

Changes from original:
- Removed font slider and all font-adjustment UI (font fixed to Arial 9).
- Removed filter popup, filter controls, and related logic.
- Removed domain A/B display toggle and subdomain counter.
- Domain viewer shows only path segments (not full URL).
- Added two case-sensitivity toggles:
    * Log search case-sensitive (checkbox)
    * Domain viewer search case-sensitive (checkbox)
- Decode percent-encoded sequences and HTML entities for displayed text.
- Keeps crawler logic, domain -> tree mapping, queue, marks, etc.
"""
import threading
import time
import json
...
class CrawlerApp:
    def __init__(self, root):
        self.root = root
        root.title("Crawler — lite")

        # Fixed font (constant)
        self._fixed_font = ("Arial", 9)
        ...
        # Tab: Log (with search and case toggle)
        ...
        # Case sensitivity toggle for log search
        self.log_case_var = tk.BooleanVar(value=False)
        self.log_case_chk = ttk.Checkbutton(log_top, text="Case-sensitive", variable=self.log_case_var, command=self._apply_log_filter)
        self.log_case_chk.pack(side=tk.LEFT, padx=(6,4))
        ...
        self.text_box = scrolledtext.ScrolledText(tb_frame, width=110, height=30, wrap=tk.NONE, font=self._fixed_font)
        ...
        # Tab: Domain Viewer (no A/B toggle, only path)
        self.tab_domain = ttk.Frame(self.notebook)
        domain_top = ttk.Frame(self.tab_domain); domain_top.pack(fill=tk.X, padx=6, pady=4)

        # Search controls (case-sensitive toggle)
        ttk.Label(domain_top, text="Search (path):").pack(side=tk.LEFT)
        ...
        # Case sensitivity toggle for domain search
        self.dom_case_var = tk.BooleanVar(value=False)
        self.dom_case_chk = ttk.Checkbutton(domain_top, text="Case-sensitive", variable=self.dom_case_var)
        self.dom_case_chk.pack(side=tk.LEFT, padx=(6,4))
        ...
        ttk.Label(domain_top, text="(paths only, starts collapsed)").pack(side=tk.LEFT, padx=(8,0))
        ...
        # Apply fixed font to Treeview via style (best-effort)
        style = ttk.Style()
        try:
            style.configure("Treeview", font=self._fixed_font)
        except Exception:
            pass
        ...
        self.queue_text = scrolledtext.ScrolledText(queue_frame, width=110, height=30, wrap=tk.NONE, font=self._fixed_font)
        ...
        self.marks_list = tk.Listbox(marks_frame, selectmode=tk.BROWSE, exportselection=False, font=self._fixed_font)
    ...
    # safe append to non-editable text; decode percent-encoding and html entities for readability
    def _text_insert(self, text, tags=()):
        safe = html.unescape(unquote(text))
        self.text_box.config(state=tk.NORMAL)
        if tags:
            self.text_box.insert(tk.END, safe + "\n", tags)
        else:
            self.text_box.insert(tk.END, safe + "\n")
        self.text_box.see(tk.END)
        self.text_box.config(state=tk.DISABLED)
        # keep history (store original text to preserve exact urls)
    ...
    def refresh_domain_view(self):
        ...
        def snapshot():
            ...
            # build nested per-domain trees (paths only)
            dom_trees = {}
            ...
            if len(children) == 1 and children[0].endswith("::DUMMY"):
                ...
                for u in sorted(tree.get("__urls", [])):
                    ...
                    if not self.domain_tree.exists(node_id):
                        ...
                        # decode display for nice characters
                        display_safe = html.unescape(unquote(display))
                        self.domain_tree.insert(iid, "end", iid=node_id, text=display_safe, tags=tags)
                # top-level segments
    ...
    def _mark_this_link(self, url):
        default = urlparse(url).path if urlparse(url).path and urlparse(url).path!="/" else url
    ...
    def domain_search_do(self):
        ...
        if not q:
            messagebox.showinfo("Empty search", "Please enter a non-empty search string (path)."); return
                ...
                for k,v in node.items():
                    if k=="__urls":
                        for u in v:
                            ...
                            if self.dom_case_var.get():
                                if q in path: matches.append((dom,u))
                            else:
                                if q.lower() in path.lower(): matches.append((dom,u))
                    else:
                        walk(v)
            walk(tree)
        if not matches:
        ...
        self._text_insert(f"[SEARCH] Found {len(matches)} matches for '{q}' (path search).")
    ...
    def refresh_queue_view(self):
        def snapshot_and_update():
            ...
            for i,u in enumerate(topn, start=1):
                ...
                lines.append(prefix + html.unescape(unquote(u)) + mark)
            def do_update():
                try:
        ...
        try:
            while True:
                ...
                if typ=="visited":
                    ...
                    # decode for display
                    t = f"Visited link: {html.unescape(unquote(url))}"
                    if is_user: t += "   [user search]"
                    if is_non: t += "   (non-html)"
                    self._append_log_history(t, is_nonhtml=is_non, is_user=is_user)
                    show = True
                    s = self.log_search_var.get().strip()
                    if s:
                        if self.log_case_var.get():
                            if s not in t: show=False
                        else:
                            if s.lower() not in t.lower(): show=False
                    if show:
                        tags = ()
                    ...
                    with self.state["lock"]:
                        ...
                        inqueue = len(self.state["pending_queue"])
                    t = f"Discovered: {html.unescape(unquote(last_url))}"
                    if is_user: t += "   [user search]"
                    if is_non: t += "   (non-html)"
                    self._append_log_history(t, is_nonhtml=is_non, is_user=is_user)
                    show = True
                    ...
                    t = f"Visited link: {html.unescape(unquote(url))}  |  skipped non-html ({ctype})"
    ...
    def _apply_log_filter(self):
        ...
        for entry in self._log_history:
            ...
            if s:
                if case:
                    if s not in text: ok = False
                else:
                    if s.lower() not in text.lower(): ok = False
            if ok:
                tags = ()
                if is_nonhtml: tags = ("nonhtml",)
                if is_user: tags = tuple(list(tags)+["usersearch"])
                # decode when inserting
                pretty = html.unescape(unquote(text))
                if tags: self.text_box.insert(tk.END, pretty + "\n", tags)
                else: self.text_box.insert(tk.END, pretty + "\n")
